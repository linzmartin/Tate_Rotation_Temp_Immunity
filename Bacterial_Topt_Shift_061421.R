...
# Model to determine effect of Thermal Optima Differences between Host and Pathogen

#Goal of this script: shift the pathogen thermal optima
###################################################################################################################
###################################################################################################################
################## SECTION 1: SETUP ##############################
###################################################################################################################
...
#Thermal Optima Differences - Model with loop to calculate shift for each temp range
...
################################
#first, want to create a shift in the thermal optimum of host immunity to explore how this affects the models:
#create list of numbers to add to thermal optimum of host immunity to create Topt shift
temp_shifts <- data.frame(temp=c(seq(from=1, to=8,by=1)),stringsAsFactors = FALSE) #a list of numbers 1-8 by increments of 1, in a data frame
...
###################################################################################################################
###################################################################################################################
######################### SECTION 2: RUN WITHIN HOST MODEL ######################
###################################################################################################################
###################################################################################################################

#run the Within Host Model with different shifts in Topt for bacteria

#must first make empty data frames first to initialize:
...
for (i in 1:length(temp_shifts$temp)){
  ...
  for (l in 1:length(fraction_data$Temp)){
    ...
    TMD <- fraction_data[l,3]
      #not using TMI in actual model
    Btoptrate<-Ratkowsky(temp=(36-i),Tmin=(6-i),Tmax=(49-i),b=0.004,c=0.14)
    Btrate <- Ratkowsky(temp=(fraction_data[l,1]),Tmin=(6-i),Tmax=(49-i),b=0.004,c=0.14)
    TB <- (Btrate/Btoptrate)
...
#Next, modify data table to work with it more easily:
appendtimes<-select(appendtimes,-2)
appendtimes$toptshift <- (36-appendtimes$toptshift)
appendtimes<-rename(appendtimes, Optimal_Temp_of_Pathogen_Bt=toptshift)
...
#Plot the outputs:
appendtimes %>%
  ggplot(aes(x=Optimal_Temp_of_Pathogen_Bt,y=daystoH50),group_by(temp))+
  geom_point()+geom_smooth(se=FALSE)+
  facet_wrap(~temp)+ 
  theme_bw()+ guides(legend)+
  labs(title="Effect of Changing Pathogen Topt on Time to 50% Health",
       y="Days to 50% Health",x="Optimal Pathogen Temperature (°C)",
       subtitle="Environmental Temperature Ranges from 24-34°C",
       caption="Beetle Host Thermal Optimum = 25.2°C")

appendtimes %>%
  ggplot(aes(x=temp,y=daystoH50),group_by(Optimal_Temp_of_Pathogen_Bt))+
  geom_point()+geom_smooth(se=FALSE)+
  facet_wrap(~Optimal_Temp_of_Pathogen_Bt)+ 
  theme_bw()+ guides(legend)+
  labs(title="Effect of Changing Pathogen Topt on Time to 50% Health",
...
appendtimes %>%
  ggplot(aes(x=Optimal_Temp_of_Pathogen_Bt,y=temp))+
  geom_raster(aes(fill=daystoH50))+
  #geom_contour(aes(z=daystoH50),colour="white")+
  labs(title="Effect of Changing Pathogen Topt on Time to 50% Health",
       y="Environmental Temperature (°C)",x="Optimal Pathogen Temperature (°C)",
       subtitle="Environmental Temperature Ranges from 24-34°C",
       caption="Beetle Host Thermal Optimum = 25.2°C",
...
############
Toptdiff<-cbind(appendtimes,Pathogen_Host_Topt_Diff=appendtimes$Optimal_Temp_of_Pathogen_Bt-25.2)
head(Toptdiff)
...
Toptdiff %>%
  ...
  labs(title="Effect of Changing Pathogen Topt on Time to 50% Health",
       ...
       subtitle="Environmental Temperature Ranges from 24-34°C",
       caption="Pathogen Thermal Optimum Ranges from 28-35°C, Host Thermal Optimum = 25.2°C")
...
###################################################################################################################
###################################################################################################################
##################  SECTION 4: INDIVID HIB (CALCS, PLOTS) & DATA EXTRACT FOR BTWN HOST MODEL ###############################
###################################################################################################################
###################################################################################################################

#separate data by variable
head(outputdataframetotals)
#need to fix this for bact topt shift:
...
###################################################################################################################
###################################################################################################################
##################### SECTION 5: SETUP FOR BETWEEN HOST MODEL ########################
###################################################################################################################
###################################################################################################################

#import the data
#also using data tables above from section 1 (make sure you don't clear ls!!!)
...
write.xlsx(savingparams, file = "Temp_dependent_params_betweenhost_with_Tempshifts.xlsx",
...
coeff_list<-coef(ratkowsky_germ_fit)
coeff_list
...
SIS_between_host_model_with_compartmentP<- function (t, x, params) {
  ...
  #parameters
  beta <- params["beta"]
  gamma <- params["gamma"] #estimated 0.5
  d <- params["d"]
  delta <- params["delta"]
  r<-params["r"]
  K<-params["K"]
  ...
  TR<-params["TR"]
  Hat24<-params["Hat24"] #Health at 24hrs post-infection (from within-host model)
  TG<-params["TG"] #fraction of optimal spore germination rate
...
###################################################################################################################
###################################################################################################################
##################### SECTION 5: RUN THE BETWEEN HOST MODEL ########################
###################################################################################################################
###################################################################################################################

#set up empty data frames first to store the data output:
SIP_outputdataframetotals<-data.frame()
SIP_out<-data.frame()
SIP_output.df <- data.frame()
#then run the between host model in a loop for temperature shifts:
...
SIP_outputdataframetotals_subset_toptexample <- subset(SIP_outputdataframetotals, toptshift==1 | toptshift==5)
