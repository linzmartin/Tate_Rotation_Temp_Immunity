#this code includes both the within-host model, the between-host model, and the modifications to 
#shift the host thermal optimum by a singular value (not a loop)
##################################################

#mismatch between host and pathogen thermal optima
#formerly named "Thermal_optima_mismatch_model"
#for individual shifts (+8C) of host thermal optimum, not a range of shifts in temp
...
Gene_data <- read_xlsx("Gene_exp_rates_Sadiq_072721.xlsx",sheet="Rate_Data")#using Sadiq's new data
...
#This is key:
#****Use "shifted_gene_data" if wanting to model shifted data
#****Use "Gene_data" if just wanting to model data without shift in temperature

fits <- Gene_data %>% #shifted_gene_data %>% 
  group_by(., Rate_Type) %>%
  nest() %>%
...
neededtemps <- Gene_data %>%
  do(., data.frame(Temp = c(seq(from=20,to=34,by=0.1)), stringsAsFactors = FALSE))
...
RateatTemp<-dplyr::select(predsforcalc, -data,-fit)
#RateatTemp<-predsforcalc %>% select(!c(data,fit))
...
for (i in 1:length(fraction_data$Temp)){
  ...
  #---> MOST UPDATED: new parms w/o Microbe Indep rate & increase alpha:
  parms <-c(psi=0.5, KH=1, beta=0.0005,p=0.0005,m=0.00001,
...
#write.xlsx(as.data.frame(output.df), file = "HIB_within_host_model_output_with_8Chostshift.xlsx",
...
#save temperature-dependent parameters:
...
tempparamdata<-read_xlsx("Temp_dependent_params_withinhost.xlsx") #import Temp param data
