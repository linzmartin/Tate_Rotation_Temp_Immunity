...
ggplot() +
  ...
  #facet_wrap(~ Gene,labeller = labeller(.multi_line = FALSE)) +
...
#positive regulators fitting models:
Gene_fits <- positive_regulators %>%
  group_by(., Treatment) %>%
  nest() %>%
  mutate(flinn = purrr::map(data, ~nls_multstart(Rate~flinn_1991(temp = Temp, a, b, c),
                                                 data = .x,
...
new_preds <- positive_regulators %>%
  do(., data.frame(Temp = seq(min(.$Temp), max(.$Temp), length.out = 150), stringsAsFactors = FALSE))

# max and min for each curve
max_min <- group_by(positive_regulators,Treatment)%>%
  summarise(., min_Temp = min(Temp), max_Temp = max(Temp)) %>%
  ungroup()
...
# plot
ggplot() +
  geom_point(aes(Temp, Rate), size=2,positive_regulators)+
  ...
  labs(x = 'Temperature (ºC)',
       y = 'Rate of Gene Expression',
       title = 'Positive Regulators Expression vs. \nTemp in Heat Killed BtU Treatment') +
  geom_hline(aes(yintercept = 0), linetype = 2) +
  scale_color_brewer(type = 'qual', palette = 2)
...
d_ic
###########################
#Negative regulators model fitting:
Gene_fits <- negative_regulators %>%
  group_by(., Treatment) %>%
  nest() %>%
  mutate(flinn = purrr::map(data, ~nls_multstart(Rate~flinn_1991(temp = Temp, a, b, c),
                                                 data = .x,
...
new_preds <- negative_regulators %>%
  do(., data.frame(Temp = seq(min(.$Temp), max(.$Temp), length.out = 150), stringsAsFactors = FALSE))

# max and min for each curve
max_min <- group_by(negative_regulators,Treatment)%>%
  summarise(., min_Temp = min(Temp), max_Temp = max(Temp)) %>%
  ungroup()
...
# plot
ggplot() +
  geom_point(aes(Temp, Rate), size=2,negative_regulators)+
  geom_line(aes(Temp,Rate,col = model_name),preds2) +
  geom_label_repel(aes(Temp, Rate, label = model_name, col = model_name), 
                   fill = 'white', nudge_y = 20, segment.size = 0.2, 
                   segment.colour = "grey50",d_labs) +
  ...
  labs(x = 'Temperature (ºC)',
       y = 'Rate of Gene Expression',
       title = 'Neg. Regulator Expression vs. Temp \nin Heat Killed BtU Treatment') +
  geom_hline(aes(yintercept = 0), linetype = 2) +
  scale_color_brewer(type = 'qual', palette = 2)
