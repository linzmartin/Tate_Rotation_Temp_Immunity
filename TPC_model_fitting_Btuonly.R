...
#Run diff models on BtU Infected group
...
max_min <- group_by(BtUInjected,Treatment,Gene)%>%
...
#separate BtUInjected Data by gene:
Cactus1_exp <- filter(BtUInjected, Gene == "Cactus1")
Relish2_exp <- filter(BtUInjected, Gene == "Relish2")
PGRPSC2_exp <- filter(BtUInjected, Gene == "PGRPSC2")
Defensin1dg2_exp <- filter(BtUInjected, Gene == "Def1dg2")
Hsp27_exp <- filter(BtUInjected, Gene == "hsp27")

#now, fit models for each gene within the Btu infected treatment group:
